...
ClickHouse recognizes the following metadata filename formats inside the
`metadata` directory:

1. `v<V>.metadata.json` – metadata version is stored in the prefix.
2. `<V>-<uuid>.metadata.json` – metadata version is stored before the UUID.
3. `<uuid>.metadata.json` – files generated by some writers like Spark. In this
   case ClickHouse treats the version as `0` when selecting metadata files.
